...
# fs_remove_site_cert <domain> — delete the per-site cert/key generated by
# fs_ensure_site_cert. Called on unbind so certs don't pile up for sites the
# user has forgotten. The shared LAN host cert (keyed on <mac>.local, not the
# domain) is left alone — it's reused across every site.
fs_remove_site_cert() {
  local cert key
  { read -r cert; read -r key; } < <(fs_cert_paths "$1")
  rm -f "$cert" "$key"
}
